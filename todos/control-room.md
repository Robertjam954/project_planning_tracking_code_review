...
## 1. Infra & databases
- [x] Config + env schema defined (`core/config.py` + `.env.example`: `ANTHROPIC_API_KEY`, `PORTFOLIO_TOKEN`, `LANGSMITH_*`)
- [x] Secrets management (no keys in git; `.env` gitignored)
- [x] State source confirmed: existing git-tracked `projects.json` / `data/history.json` / `todos/*.md` (no new DB)
...
## 2. Agents
- [x] Agent roster + responsibilities defined (supervisor + planner + tracker + reviewer + historian)
- [x] Supervisor/router `StateGraph` implemented (`agents/graph.py`, `agents/nodes.py`)
- [x] Report-back contract: workers return a structured result to the supervisor
- [x] Model + params per agent (`claude-sonnet-5`); step/recursion cap enforced
- [x] Historian worker answers "what did we discuss" from durable conversation memory

## 3. Tools
- [x] Tools enumerated with input schemas (`new_agentic_project`, `build_dashboard`, `sync_status`, `read_status`, `read_history`, `fetch_pr_reviews`, `recall_history`)
- [ ] Existing scripts made importable (`main()` callable) without changing CLI behavior
- [x] Registered in `agents/tools.py`; per-agent tool subsets assigned
- [ ] `gh api` auth for `fetch_pr_reviews`; timeouts + error handling (degrade, don't crash)
- [ ] Unit test per tool (mock `subprocess` / `gh`)

## 4. Memory
- [x] Short-term: `MessagesState` wired; multi-turn within a session
- [x] Checkpointer (SQLite `SqliteSaver`, `data/agent_checkpoints.db`) keyed by session/thread id
- [x] Durable conversation store (`agents/memory.py` `ConversationStore`, SQLite `data/agent_memory.db`): every turn persisted across sessions
- [x] Queryable recall: `recall_history` tool (list sessions, read a session transcript, keyword search) surfaced via the historian worker
- [ ] History window / summarization strategy (only if sessions grow long)

## 5. Prompts
- [x] Prompt registry `agents/prompts.py` (separate from code)
- [x] One system prompt per role: supervisor router, planner, tracker, reviewer, historian
- [x] Variables/placeholders documented; versioned in git
...
- [ ] (Optional) chat surface - React surface scaffolded in `frontend/` (markdown-messages pattern, LangGraph SDK); pending install + smoke test - see [`control-room-frontend.md`](control-room-frontend.md)
- [x] Auth on the surface - N/A - local CLI

## 7. Tracing / observability / eval
- [x] Per-node + per-tool LangSmith spans (`agents/tracing.py`, `LANGCHAIN_TRACING_V2`)
